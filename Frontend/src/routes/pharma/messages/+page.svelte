<script lang="ts">
	...
	// Hardcoded trial ID for now - in production, get from session/context
	const TRIAL_ID = 'TRIAL-2026-003';
	const PHARMA_DID = 'did:t3n:pharma'; // Placeholder
	...
	onMount(async () => {
		...
		// Check if we need to initiate a new conversation from query params
		const urlTrialId = $page.url.searchParams.get('trialId');
		const urlPatientDid = $page.url.searchParams.get('patientDid');
		
		if (urlTrialId && urlPatientDid) {
			// Check if conversation exists
			const existingConv = conversations.find(c => c.trialId === urlTrialId && c.patientDid === urlPatientDid);
			if (existingConv) {
				await selectConversation(existingConv);
			} else {
				// Create placeholder conversation (will be created when first message is sent)
				const newConv: Conversation = {
					_id: `${urlTrialId}-${urlPatientDid}`,
					trialId: urlTrialId,
					trialName: urlTrialId, // We'll get the real name when sending first message
					patientDid: urlPatientDid,
					lastMessage: '',
					lastMessageTime: new Date().toISOString(),
					lastSenderType: 'pharma',
					unreadCount: 0
				};
	...
	async function fetchConversations() {
		try {
			const response = await fetch(`http://localhost:3008/api/messages/pharma/conversations?trialId=${TRIAL_ID}`);
	...
	async function markAsRead(trialId: string, patientDid: string) {
		try {
			await fetch('http://localhost:3008/api/messages/mark-read', {
				...
				body: JSON.stringify({ trialId, patientDid, readerType: 'pharma' })
			});

			// Update conversation unread count locally
			const conv = conversations.find(c => c.trialId === trialId && c.patientDid === patientDid);
		...
		try {
			...
			const response = await fetch('http://localhost:3008/api/messages/send', {
				...
				body: JSON.stringify({
					...
					patientDid: selectedConversation.patientDid,
					senderId: PHARMA_DID,
					senderType: 'pharma',
...
<TopBar title="Messages" showSearch={false} userType="pharma" userId={TRIAL_ID} />
			...
			{:else if conversations.length === 0}
				<div class="flex flex-col items-center justify-center py-12 px-4 text-center">
					...
					<p class="text-label-sm text-on-surface-variant mt-2">
						Initiate a chat from match results
					</p>
				</div>
			{:else}
				{#each conversations as conversation}
					<button
						...
						class:bg-[var(--color-tm-elevated)]={selectedConversation?.patientDid === conversation.patientDid}
					...
					>
						<div class="flex items-start justify-between gap-2 mb-1">
							<div class="flex items-center gap-2 flex-1 min-w-0">
								...
								<p class="text-body-md font-medium text-on-surface truncate">
									{conversation.patientDid.slice(0, 20)}...
								</p>
							</div>
						...
						<div class="flex items-center justify-between gap-2">
							<p class="text-label-sm text-on-surface-variant truncate flex-1">
								{#if conversation.lastSenderType === 'pharma'}
		...
		{#if !selectedConversation}
			<div class="flex items-center justify-center h-full">
				<div class="text-center">
					...
					<p class="text-label-sm text-on-surface-variant mt-2">Choose a patient to start messaging</p>
				</div>
			</div>
		{:else}
			<!-- Chat Header -->
			<div class="p-4 border-b border-[var(--color-tm-border)] bg-[var(--color-tm-surface)]">
				<div class="flex items-center gap-3">
					...
					<div class="flex-1 min-w-0">
						<p class="text-body-md font-medium text-on-surface truncate">
							{selectedConversation.patientDid.slice(0, 30)}...
						</p>
			...
			<div id="messages-container" class="flex-1 overflow-y-auto p-4 space-y-4">
				...
				{:else}
					{#each messages as message}
						{@const isOwnMessage = message.senderType === 'pharma'}
